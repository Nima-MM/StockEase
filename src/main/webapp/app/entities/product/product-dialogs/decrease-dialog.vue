    ...
    <Dialog v-model:visible="visible" pt:root:class="!border-0 !bg-transparent" pt:mask:class="backdrop-blur-sm">
      <template #container="{ closeCallback }">
        <div
          ...
          style="background-image: radial-gradient(circle at left top, red, red, orange, yellow)"
        >
          ...
          <div class="inline-flex flex-col gap-2">
            <p>
              <span class="text-stone-950">{{ product?.category?.name }} - {{ product?.color?.name }}</span
              ><br />
          ...
          <div class="inline-flex flex-col gap-2">
            <label for="amount" class="text-stone-950 font-semibold">Um wie viel reduzieren?</label>
            <InputNumber v-model="amount" id="amount" class="!bg-white/10 !border-0 !p-4 !text-primary-50 w-80" />
          </div>
          <div class="flex items-center gap-4">
            <Button
              ...
              @click="closeCallback"
              text
              class="!p-4 w-full !text-stone-950 !border !border-white/50 hover:!bg-white/25"
            ></Button>
            <Button
              label="Reduzieren"
...
<!-- <dialog-template
  ...
  header-icon="mdi-archive-minus"
  header-title="BESTAND REDUZIEREN"
>
  -- Activator Slot --
  ...
  <template #input>
    <div style="display: flex; gap: 10%">
      <p style="color: #e64a19; padding: 18px; font-size: large; font-weight: bold">Um wie viel reduzieren?</p>
  ...
  <template #actions="{ close }">
    ...
    <v-spacer></v-spacer>
    <v-btn color="#E64A19" @click="() => confirmReduction(close)">Reduzieren</v-btn>
  </template>
</dialog-template> -->
