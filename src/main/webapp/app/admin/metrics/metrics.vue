<template>
  <div>
    <h2>
      <span id="metrics-page-heading" data-cy="metricsPageHeading">Anwendungs-Metriken</span>
      <button class="btn btn-primary float-right" @click="refresh()">
        <font-awesome-icon icon="sync"></font-awesome-icon> <span>Aktualisieren</span>
      </button>
    </h2>
    ...
    <div class="row" v-if="!updatingMetrics">
      ...
      <div class="col-md-4">
        ...
        <span><span>Wartezeit</span> ({{ threadStats.threadDumpTimedWaiting }})</span>
        <b-progress variant="success" :max="threadStats.threadDumpAll" striped>
          <b-progress-bar
        ...
        <span><span>Wartend</span> ({{ threadStats.threadDumpWaiting }})</span>
        <b-progress variant="success" :max="threadStats.threadDumpAll" striped>
          <b-progress-bar
        ...
        <span><span>Geblockt</span> ({{ threadStats.threadDumpBlocked }})</span>
        <b-progress variant="success" :max="threadStats.threadDumpAll" striped>
          <b-progress-bar
        ...
        <table class="table table-striped" aria-describedby="Jvm gc">
          <thead>
            <tr>
              ...
              <th scope="col" class="text-right">Durchschnitt</th>
    ...
    <h3>HTTP Anfragen (Ereignisse pro Sekunde)</h3>
    <table
    ...
    >
      <thead>
        <tr>
          <th scope="col">Codierung</th>
          <th scope="col">Anzahl</th>
          <th scope="col" class="text-right">Durchschnittswert</th>
    ...
    <h3>Datenquelle (Zeit in Millisekunden)</h3>
    <div class="table-responsive" v-if="!updatingMetrics && isObjectExistingAndNotEmpty(metrics, 'databases')">
      <table class="table table-striped" aria-describedby="Connection pool">
        <thead>
          <tr>
            <th scope="col">
              <span>Usage</span> (active: {{ metrics.databases.active.value }}, min: {{ metrics.databases.min.value }}, max:
              {{ metrics.databases.max.value }}, idle: {{ metrics.databases.idle.value }})
            </th>
            <th scope="col" class="text-right">Anzahl</th>
            <th scope="col" class="text-right">Mittel</th>
            <th scope="col" class="text-right">Minimum</th>
            <th scope="col" class="text-right">p50</th>
            <th scope="col" class="text-right">p75</th>
            <th scope="col" class="text-right">p95</th>
